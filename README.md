...
![最高支持](https://img.shields.io/badge/最高支持-Phigros%203.19.2-blue.svg)
...
- 截止目前，项目作者与fork作者从未在任何除 GitHub 以外的平台上以任何方式宣传过本项目
...
![Phisap截图](./public/img/screenshots/phisap_interface.webp)
...
3. 请准备 Phigros 的游戏安装包/通用数据包。目前fork作者也不清楚支持的情况怎么样，但是最新版应该是支持的，如3.19.2。往前的版本可能在解包上出现兼容性问题。不过可以通过加载自定义谱面解决。
...
4. 准备服务端。请去[scrcpy 的 releases 页面](https://github.com/Genymobile/scrcpy/releases) 下载`scrcpy-server-v4.0`，不要下载成别的版本。下载完成后，请将文件直接放置在 phisap 的根目录（与`main.py`之类的文件在同一文件夹即可），不要更改文件的名称（比如添加后缀），否则 phisap 将无法识别
   - 如果你使用\*nix 系统，且安装有 wget，那么下面的命令与上面描述的操作等效:
     ```bash
     cd phisap  # 定位到phisap的根目录下
     wget https://github.com/Genymobile/scrcpy/releases/download/v4.0/scrcpy-server-v4.0
     ```
...
- **phisap 当前（可能完美，待测试ing）支持的最高版本为 3.19.2**，所有的曲目/任意难度均可以 AP，除了一些特殊类型的谱面（不用担心，不会影响 rks）。这些谱面详见[暂不支持的谱面](#暂不支持的谱面)。不过，在使用 phisap 时可能会发现一些谱面无法 AP，那么这时你需要
...
- `catalog.py`和`extract.py`中的代码参考自[Perfare/AssetStudio](https://github.com/Perfare/AssetStudio)，以及[7aGiven/Phigros_Resource](https://github.com/7aGiven/Phigros_Resource)
...
- **更新了下algo4**
  - 大概改了修了修几处几角旮旯的地方
  - 然后愚人节谱面基本上支持了，只差雪降了
  - 算法上更新了一点，主要是将之前的判定区算法重写了一下
  - 还把数据抓出来了，现在判定宽度等数据应该没问题了
  - 可能还需要一点时间测试，不知道会不会搞出什么bug
  - 那就先端上来吧
...
- **修复缓存复用的问题**
  ...
  - 但是还没有校验算法的SHA，唉算了先不搞了

### (2026/06/14) v0.2.1

- **修复大量GUI问题**
  - 将算法求解移至独立线程 `SolveWorker`，避免在规划谱面时阻塞主线程
  - 以及一些杂七杂八的小问题。总之我也不记得了
- **修复OTG后端**
  - 增加了AOAv2协议注册的容错处理
  - 修复了谱面结束后会导致再次autoplay失效的bug
  - 修复所有触控在X方向偏移的问题。对屏幕进行归一化处理
  	- 注意，OTG/HID后端通常只推荐在类Unix系统上使用。Windows上由于ADB驱动的强制占用，导致此后端无法进行底层通信
  	- 尽管可以使用Zadig进行驱动替换操作，但仍然不建议普通用户使用
  
- **正式完成Algo 4算法**
  - 此算法通过几何空间计算，对note的判定区域进行求解。然后据此利用Phigros一些技巧来进行优化。                                                  包括：
    - 对Flick的滑动方向进行动态选择。Flick会优先选择Drag/Hold数多的方向
    - 对Tap与Hold头进行重叠规避处理
    - 将不可视的note通过垂直判定已经微调时间的方式转入进屏幕里
    - Drag/Hold类音符的指针优化。即当有Pointer在此类note的判定区域内，直接忽略掉，以此来节省指针
  - 就目前来说，algo4已经可用了。并且在测试谱面中表现良好。后续将持续对算法进行重构与优化

### (2026/06/08) v0.2.0

- Pgr 3.19.2
  - fork项目建立。项目仍然遵循WTFPL协议 
  - 对大量代码进行了重构
  - **修复在Autoplay时，因为提前终止进程，而导致下一次autoplay触控完全失效的问题**
  	- 先前的algo都会在谱面的末尾添加对10个指针的UP事件，但是提前终止autoplay就导致这10个指针部分还残留在屏幕上，影响下一次的worker
  - 将`scrcpy`的版本从 2.0 提升到了 4.0 ，并更进了`scrcpy`服务端的API
  	- 主要是一些client与scrcpy-server之间的视频流约定与解码的变化
  	- 这样同时添加了对Android 15/16及以后的支持
  	- **注意OTG实现暂未变化，但后续应该会有对OTG的支持**
  - **添加新算法algo4**：
  	- 没想好啥名字，就叫做优化算法吧。当然algo4更直接一点
  	- 目前已经加入GUI中，其配置项与algo1相似，推荐的配置是：
  		- 红键开始于 -20ms
  		- 红键结束于 +20ms
  		- 采样间隔为 5ms
  	- 也可以根据谱面Hold、Flick的密度自行手动调整
  	- 此算法融合了前三个算法的特点，目的是最大化优化ACC，并引入了很多细节以及边界检查
  	- 下面简单介绍一些细节
  		- 待补充
  	- 就目前来看，algo4有望成为最优算法，但仍在施工中
  - **修复对新版本Phigros的解包**
  	- 此修改暂未确定其对早版本Phigros解包的影响，可能会导致较早期的Phigros解包失败（比如3.13即以前），但暂未测试
  - 将依赖从requirements.txt转进pyproject.toml里，优化了下项目结构
  	- 修改后，**建议通过uv来安装项目**
  		- ```uv sync```
  - **重构preview.py**
  	- 完全开发成一个指针可视化与谱面简洁渲染的工具
  	- 此改动引入了新依赖`skia-python`，安装方法同上
  	- 移除了对`pyglet`的依赖。使用`skia`渲染，同时大幅度提升了流畅性
  	- 添加了一些常用操作说明
  	- 目前只支持对algo4进行检验
  	- 为note添加了点颜色与区分度，同时将指针渲染为红色圆圈，方便debug算法
  	- 简单是简单了点，但是应该算不上简陋。算是比较实在的工具吧
  	- 目前是跑在CPU上的。稍稍有点慢，但是平常调试都是0.2x倍速播放，所以不会考虑GPU支持
  	- ~~注意目前对Hold的渲染还存在bug，可能会导致部分note未被渲染。暂未找到原因，我猜测可能和某些优化有关~~
  	  - 已修复。判定线的透明度不影响Note渲染的透明度
  - 清理项目结构，修改README.md
  - **将依赖从PyQt5替换为PySide6**
  	- API几乎不需要变动，主要是提升了下在Windows下的兼容性，`PyQt5`不知道为什么在`uv`与Windows环境下安装有问题
  - 将public下的截图压缩成webp格式，节约空间与在线阅读README.md的流量
  	- 增加了下项目主页图片的加载速度 
  - 重命名dev分支为main分支，移除其他全部分支
